...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        ...
        frictiveMove() {
            ...
            this.temp = (Math.abs(this.xmom)+Math.abs(this.ymom))*100
        }
        frictiveunMove() {
            if (this.reflect == 1) {
                if (this.x + this.radius > canvas.width) {
    ...
    function setUp(canvas_pass, style = "#090909") {
        ...
        function continued_stimuli(e) {
            ...
            TIP_engine.y = YS_engine
            TIP_engine.body = TIP_engine
            boilseten.source.x = TIP_engine.x
            boilseten.sparkclock++
            boilseten.source.y = TIP_engine.y
        }
    }
    ...
    class Boiler{
        ...
        draw(){
            ...
            if(fire == 1){
                this.source.draw()
                this.sparkclock++
                if(this.sparkclock%13 == 0 || this.sparkclock%7 == 0 || this.sparkclock%17 == 0){
                    let spark = new Circle(this.source.x, this.source.y, 2, "#FFAA0088", (Math.random()-.5)*2, -3)
            ...
            for(let t = 0;t<this.sparks.length;t++){
                if(this.shell.isPointInside(this.sparks[t])){
                    this.sparks[t].marked = 1

                    let min = 100000
                    let index = -1
                    for(let k = 0;k<this.particles.length;k++){
                        let link = new LineOP(this.particles[k], this.sparks[t])
                        if(link.sqrDis() < min){
                            min = link.sqrDis()
                            index = k
                        }
            ...
            for(let t = 0;t<this.particles.length;t++){
                ...
                for(let t = 0;t<this.particles.length;t++){
                    this.particles[t].wet = 0
                    for(let k = 0;k<this.particles.length;k++){
                        if(t!=k){
                            let link = new LineOP(this.particles[t],this.particles[k])
                            if(link.sqrDis()<35+((this.particles[t].temp+this.particles[k].temp)*.2)){
                                this.particles[t].wet = 1
                                // this.particles[t].ymom*=.9
                                // this.particles[k].ymom*=.9
                                this.particles[t].xmom+=(this.particles[t].x-this.particles[k].x)/50
                                this.particles[t].ymom+=(this.particles[t].y-this.particles[k].y)/50
                                this.particles[k].xmom-=(this.particles[t].x-this.particles[k].x)/50
                                this.particles[k].ymom-=(this.particles[t].y-this.particles[k].y)/50

                                let tsto = {}
                                tsto.xmom = this.particles[t].xmom
                                tsto.ymom = this.particles[t].ymom

                                this.particles[t].xmom = ((this.particles[t].xmom*19) + this.particles[k].xmom)*.05
                                this.particles[t].ymom = ((this.particles[t].ymom*19) + this.particles[k].ymom)*.05

                                this.particles[k].xmom = ((this.particles[k].xmom*19) + tsto.xmom)*.05
                                this.particles[k].ymom = ((this.particles[k].ymom*19) + tsto.ymom)*.05


                            if(this.particles[t].doesPerimeterTouch(this.particles[k])){
                                this.particles[t].xmom+=(this.particles[t].x-this.particles[k].x)/4
                                this.particles[t].ymom+=(this.particles[t].y-this.particles[k].y)/4
                                this.particles[k].xmom-=(this.particles[t].x-this.particles[k].x)/4
                                this.particles[k].ymom-=(this.particles[t].y-this.particles[k].y)/4
                            }
                            }
                        }
                    }
                    if(this.particles[t].wet == 0){
                        this.particles[t].ymom-=.19
                        this.particles[t].color = "#CCCCFF44"
                    }else{
                        this.particles[t].color = "#00AAFF44"
                    }
                }
        }
    }
